...
struct PopupView: View {
    ...
    var body: some View {
        if popupVM.isPresented {
            if self.popupVM.popupCase == .loadControl {
                ZStack {
                    MBProgressHUDRepresentable(percent: self.$popupVM.percentages, show: self.$popupVM.isPresented, completion: {self.popupVM.percentages = 0.0})
                }
